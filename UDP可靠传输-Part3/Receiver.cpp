...
// 假设路由器的缓冲区（缓冲队列）线程一直接收，1024*1024*10
queue<my_udp> router_buffer;
...
DWORD WINAPI recv_router(LPVOID lparam_router) {
    int iResult = 0;
    bool flag = true;
    router* temp_router = (router*)lparam_router;

    while (flag) {
        char* RecvBuf = new char[UDP_LEN]();
        my_udp temp;
        iResult = recvfrom(temp_router->RecvSocket, RecvBuf, UDP_LEN, 0, (SOCKADDR*)&(temp_router->SenderAddr), &(temp_router->SenderAddrSize));
        if (iResult == SOCKET_ERROR) {
            cout << "Recvfrom failed with error: " << WSAGetLastError() << endl;
        }
        else if (router_buffer.size() <= MAX_ROUTER) {
            memcpy(&temp, RecvBuf, UDP_LEN);

            // 新获取的数据分组入队
            router_buffer.push(temp);
            // if (temp.udp_header.Flag == OVER)
                // return 1;
        }
        else;

        if (router_buffer.empty()) {
            flag = false;
        }
        delete[] RecvBuf;
    }
...
DWORD WINAPI recv_file_Reno(LPVOID lparam_router) {
    ...
    while (flag) {
        Sleep(20); // 模拟发送延迟，可以设置成随机值

        if(!router_buffer.empty()){
            my_udp temp = router_buffer.front();
            router_buffer.pop();
            ...
            */
            ...
            if (temp.udp_header.Flag == START) {
                ...
                if (checksum((uint16_t*)&temp, UDP_LEN) != 0 || temp.udp_header.SEQ != uint16_t(seq_order)) {
                    ...
                    Send_ACK(temp_router->RecvSocket, temp_router->SenderAddr, temp_router->SenderAddrSize);
...
int main()
{
    ...
    // 确定发送的addr_in
    sockaddr_in SenderAddr;
    memset((char*)&SenderAddr, 0, sizeof(SenderAddr)); // 初始化，我靠
    int SenderAddrSize = sizeof(SenderAddr);
    ...
    while (true) {
        ...
        if (ready2quit == 0) {
            ...
            /*
            router test_router;
            test_router.RecvSocket = RecvSocket;
            test_router.SenderAddr = SenderAddr;
            test_router.SenderAddrSize = SenderAddrSize;

            HANDLE hThread[2];
            hThread[0] = CreateThread(NULL, 0, recv_router, (LPVOID)&test_router, 0, NULL);
            hThread[1] = CreateThread(NULL, 0, recv_file_Reno, (LPVOID)&test_router, 0, NULL);
